...
Для зарегистрированного receipt warning показывает observed, required и
missing validation power, а также eligible участников эпохи receipt, которые
не подписали его. Если observed power уже достиг required, но статус всё ещё
`BRIDGE_PENDING`, монитор отправляет отдельный сигнал о проблеме финализации.
Ошибка всех Gonka API отслеживается отдельно и не считается зависшей
транзакцией.
Перед удалением завершённой транзакции из очереди монитор сохраняет её эпоху,
список подписавших validators и snapshot eligible участников с их весами.
Последние 20 таких записей используются как фактическое liveness-доказательство
для Top-10 bridge peers.
...
- индивидуальный warning, если участник Top-10 по BLS slots отсутствует среди
  подписантов двух последних завершённых bridge-транзакций, которые он имел
  право подписать согласно сохранённому snapshot эпохи каждого receipt;
...
Top-10 peer: он остаётся только диагностическим сигналом `unknown`. Для быстрой
реакции проверяются самые новые completed transactions текущей и предыдущей
эпохи, но каждая транзакция учитывается для конкретного peer только при наличии
его адреса в сохранённом eligibility snapshot этого receipt. Это сохраняет
быструю проверку последних транзакций и не создаёт ложное обвинение при переходе
эпох в доступном протоколом двухэпоховом окне подписи. Для старых записей без
snapshot действует безопасный fallback: только текущая эпоха и текущий eligible
состав. Если подходящих завершённых транзакций меньше двух, состояние подписей
считается неизвестным и индивидуальный warning не отправляется.
...
Для события `left` Telegram сообщает, что хост больше не участвует в текущей
эпохе, и показывает последнюю активную эпоху. Малая положительная доля веса
отображается как `<0.1%`, а не округляется до `0.0%`.
...
`/models`, `/excluded`, `/devshard`, `/gateways` (и русские алиасы)
